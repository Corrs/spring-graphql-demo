...
public class DynamicJobController {
    ...
    @QueryMapping
    public Connection<DynamicJobVO> dynamicJobs() {
        return RelayUtil.build(null);
    }
    ...
    @Valid
    @MutationMapping
    public boolean pauseDynamicJob(@Argument @NotNull(message = "任务标识不能为空")
                                   @Min(value = 1, message = "任务不存在") Long id) {
        return dynamicJobService.pause(id);
    }
    ...
    @Valid
    @MutationMapping
    public boolean resumeDynamicJob(@Argument @NotNull(message = "任务标识不能为空")
                                    @Min(value = 1, message = "任务不存在") Long id) {
        return dynamicJobService.resume(id);
    }
}
